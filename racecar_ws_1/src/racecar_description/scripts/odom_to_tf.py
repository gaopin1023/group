#!/usr/bin/env python3
"""
统一发布 racecar 相关动态 TF（全部带 racecar/ 前缀）：
  racecar/odom → racecar/base_link            (来自 /odom 里程计，50Hz)
  racecar/base_link → racecar/base_link/laser (激光 frame 别名，偏移 0 0 0.05)
  racecar/base_link → racecar/base_link/camera(相机 frame 别名，偏移 0 0 0.12)
全部使用 /odom 的仿真时间戳。
...
class OdomToTF(Node):
    ...
    def odom_callback(self, msg: Odometry):
        ...
        # 1. {odom_frame} → racecar/base_link（里程计位姿）
        t1 = TransformStamped()
        t1.header.stamp = now
        t1.header.frame_id = odom_frame
        t1.child_frame_id = 'racecar/base_link'
        ...
        # 2. racecar/base_link → racecar/base_link/laser（偏移 0 0 0.05）
        t_laser = TransformStamped()
        t_laser.header.stamp = now
        t_laser.header.frame_id = 'racecar/base_link'
        ...
        # 3. racecar/base_link → racecar/base_link/camera（偏移 0 0 0.12）
        t_camera = TransformStamped()
        t_camera.header.stamp = now
        t_camera.header.frame_id = 'racecar/base_link'
